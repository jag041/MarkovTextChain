...
struct Node {
    ...
    void addChild(size_t _node) { 
        ...
        weights.push_back(1.f / (float)numChildren);
    ...
    void createDictionary(unsigned int kl) {
        ...
        while (true) {
            ...
            if (w1.length() + 2 >= (fileBuffer.length() - pos))
                break;
            std::string w11 = key.substr(key.find_first_of(' ', 0) + 1, key.size() - 1);
            key = w11 + " " + w1;
            // std::cout << "key 2: " << key << std::endl;
            size_t secondNode = addNode(w11, w1);
